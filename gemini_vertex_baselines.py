...
# Ensure required credentials are set based on mode (Vertex vs public API)
if USE_VERTEX:
...
RESULTS_XLSX = "finchart_gemini_vertex_results.xlsx"   # Results saved here
RATE_LIMIT_DELAY = 0.2  # To avoid rate limit errors

# ------------------------------
# Normalization helpers
# ------------------------------
def safe_float(x):
    """Extract numeric value from prediction string (used for QA)."""
    try:
...
def normalize_tf(pred):
    """Normalize True/False predictions to 'true' or 'false'."""
    p = str(pred).lower()
    p = re.sub(r'[^a-z]', '', p)  # strip non-letters
...
def normalize_mc(pred):
    """Normalize multiple-choice answers to a/b/c/d."""
    p = str(pred).lower()
...
def acc(predicts, ground_truth, qtype):
    """Compute accuracy for TF, MC, QA questions."""
    correct = 0
    for p, gt in zip(predicts, ground_truth):
        if qtype == "QA":
            p = safe_float(p); gt = safe_float(gt)
            if p is None or gt is None: continue
        elif qtype == "TF":
...
def clean_image_name(img_name):
    """Fix inconsistent image naming conventions (_q1.jpg → .jpg)."""
...
def load_local_unified_dataset(test_folder, MC_json, QA_json, TF_json, limit=1000):
    """Load MC, QA, TF questions with corresponding images into a unified dataset."""
    unified = []
...
def cache_key(model, question, image_path, qtype=None):
    """Generate unique key for cache based on model, question, and image."""
...
def generate_content_cached(model, question, image_path, qtype=None):
    """Return cached prediction if available, otherwise call API and cache it."""
...
def upsert_results_excel(xlsx_path, row_dict):
    """Append model evaluation results into an Excel file."""
...
def make_gemini_client():
    """Create a Gemini client (Vertex AI or API key)."""
    if USE_VERTEX:
...
def maybe_resize(image_path, max_side=1024):
    """Resize image if larger than max_side to reduce API payload size."""
    try:
...
def build_content_parts(question, image_path=None):
    """Build parts (text + optional image) for Gemini API request."""
    parts = []
    if image_path:
...
def generate_content(model, question, image_path=None, max_retries=5):
    """Call Gemini model to generate prediction (with retries for 503 errors)."""
...
def evaluate_split(model, split_set):
    """Evaluate a model on a dataset split (test set)."""
    preds, gts, types_ = [], [], []
    total = len(split_set)
